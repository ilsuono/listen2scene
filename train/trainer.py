    ...
    def train(self, data_loader, stage=1):
        ...
        for epoch in range(self.max_epoch):
            ...
            for i, data in enumerate(data_loader, 0):
                ...
                netG.zero_grad()
                mesh_net.zero_grad()
                errG,L1_error,divergence_loss,MSE_error1,MSE_error2,criterion_loss= compute_generator_loss(epoch,netD,real_RIRs, fake_RIRs,
            ...
            print("Divergence errors ",divergence_loss)
            print("MSE error1  ",MSE_error1)
            print("MSE error2 ",MSE_error2)
            print("criterion_loss  ",criterion_loss)

            store_to_file ="[{}/{}][{}/{}] Loss_D: {:.4f} Loss_G: {:.4f} Loss_real: {:.4f} Loss_wrong:{:.4f} Loss_fake {:.4f}  MSE Error:{:.4f}  Divergence Error:{:.4f}  MSE Error1:{:.4f}  MSE Error2:{:.4f}  criterion_loss:{:.4f} Total Time: {:.2f}sec".format(epoch, self.max_epoch, i, len(data_loader),
                     errD.data, errG.data, errD_real, errD_wrong, errD_fake,L1_error*4096,divergence_loss,MSE_error1,MSE_error2,criterion_loss, (end_t - start_t))
